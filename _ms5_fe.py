# -*- coding: utf-8 -*-
import os, sys, json, base64
sys.stdout.reconfigure(encoding="utf-8", errors="replace")
import paramiko
HOST, USER, PWD = "111.229.7.15", "root", os.environ.get("SSHPW", "")
PATHX = "export PATH=/usr/local/bin:/usr/bin:/bin:$PATH; "
W = "/www/wwwroot/plm-web"; SITE = "/www/wwwroot/lab.dhupi.cn"
cli = paramiko.SSHClient(); cli.set_missing_host_key_policy(paramiko.AutoAddPolicy())
cli.connect(HOST, 22, USER, PWD, timeout=15, look_for_keys=False, allow_agent=False)
print("=== SSH OK ===")
def run(cmd, t=480):
    i, o, e = cli.exec_command(cmd, timeout=t); return o.read().decode("utf-8","replace").rstrip(), e.read().decode("utf-8","replace").rstrip()
def step(t, c, to=480):
    o, e = run(c, to); print("\n#### %s"%t); print(o[-1500:])
    if e: print("[stderr]", e[-700:])
def wfile(path, content):
    b = base64.b64encode(content.encode()).decode()
    o,e=run("mkdir -p $(dirname %s) && python3 - <<'PY'\nimport base64\nopen(%r,'w',encoding='utf-8').write(base64.b64decode('%s').decode())\nprint('w ok')\nPY"%(path,path,b))
    print("  写", path.replace(W,""), o.strip(), e[-150:])
def pyedit(path, reps):
    b = base64.b64encode(json.dumps(reps, ensure_ascii=False).encode()).decode()
    o, e = run("python3 - <<'PYEOF'\nimport base64,json\np=%r\nreps=json.loads(base64.b64decode('%s').decode())\ns=open(p,encoding='utf-8').read()\nfor a,b in reps:\n  assert a in s,('MISS '+a[:80])\n  s=s.replace(a,b,1)\nopen(p,'w',encoding='utf-8').write(s)\nprint('ok')\nPYEOF"%(path,b))
    print("  edit", path.split('/')[-1], o.strip(), e[-220:])

# ===== Schedule.tsx 全量重写（动态tab + 记录） =====
wfile(W + "/src/pages/Schedule.tsx", r"""import { useEffect, useState } from 'react'
import http from '../lib/api'
import { useAuth } from '../store/auth'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Button } from '@/components/ui/button'

interface Inst { id: number; name: string; category: string; blockType: string; piggyback?: boolean; lottery?: boolean }
interface Cat { key: string; label: string; lottery: boolean; sortOrder: number }
interface Bk { id: number; instrumentId: number; userId: number; userName: string; date: string; startHour: number; endHour: number; taskType: string; tempCeiling: number | null; source: string; fromName: string | null }
const WD = ['周一', '周二', '周三', '周四', '周五', '周六', '周日']
const TASK: Record<string, string> = { FILM: '铺膜', DRY: '干燥', FULL_DAY: '全天', HALF_DAY: '半天', BLOCK: '占用' }
const LAYOUT: Record<string, number[][]> = { FOUR_HOUR: [[8, 12], [12, 16], [16, 20], [20, 24]], HALF_DAY: [[8, 16], [16, 24]], FULL_DAY: [[8, 24]] }
function addDays(iso: string, n: number) { const d = new Date(iso + 'T00:00:00Z'); d.setUTCDate(d.getUTCDate() + n); return d.toISOString().slice(0, 10) }
function curMonday() { const d = new Date(); const wd = d.getDay(); d.setDate(d.getDate() + (wd === 0 ? -6 : 1 - wd)); return d.toISOString().slice(0, 10) }

export default function Schedule() {
  const { user } = useAuth()
  const isStudent = user?.role === 'STUDENT'
  const isAdmin = user?.role === 'ADMIN'
  const myId = (user as any)?.id as number
  const today0 = curMonday()
  const maxStu = addDays(today0, 7)

  const [insts, setInsts] = useState<Inst[]>([])
  const [cats, setCats] = useState<Cat[]>([])
  const [myHeads, setMyHeads] = useState<number[]>([])
  const [week, setWeek] = useState('')
  const [cat, setCat] = useState('VACUUM_OVEN')
  const [bks, setBks] = useState<Bk[]>([])
  const [msg, setMsg] = useState('')
  const [busy, setBusy] = useState(false)
  const [piggConfirm, setPiggConfirm] = useState<number | null>(null)

  useEffect(() => {
    http.get('/instruments').then((d: any) => setInsts(d))
    http.get('/instruments/categories').then((d: any) => setCats(d)).catch(() => {})
    http.get('/instruments/my-heads').then((d: any) => setMyHeads(d.map((x: Inst) => x.id))).catch(() => {})
    if (isStudent) setWeek(today0)
    else http.get('/instruments/booking/cycle').then((c: any) => setWeek(c.cycleKey || today0))
  }, [])
  async function loadBks(wk: string) { if (!wk) return; const d: any = await http.get('/instruments/lottery/result?cycle=' + wk); setBks(d.items || []) }
  useEffect(() => { loadBks(week) }, [week])

  const prevDisabled = isStudent && week <= today0
  const nextDisabled = isStudent && week >= maxStu
  function go(delta: number) { const w = addDays(week, delta); if (isStudent && (w < today0 || w > maxStu)) return; setWeek(w); setMsg('') }

  async function runLottery() {
    if (!window.confirm('对本周(' + week + ')运行抽签分配？')) return
    setBusy(true)
    try { const r: any = await http.post('/instruments/lottery/run', { cycle: week }); setMsg('✅ 抽签完成，生成 ' + r.bookings + ' 条' + (r.settled ? '（已结算优先级）' : '（重排）')); loadBks(week) }
    catch (e: any) { setMsg('✖ ' + e.message) } finally { setBusy(false) }
  }
  async function claim(inst: Inst, date: string, s: number, e: number) {
    setMsg('')
    try { await http.post('/instruments/booking/claim', { instrumentId: inst.id, date, startHour: s, endHour: e }); loadBks(week) }
    catch (err: any) { setMsg('✖ ' + err.message) }
  }
  async function release(b: Bk) {
    if (!window.confirm('删除这条记录/预约？')) return
    try { await http.delete('/instruments/booking/claim/' + b.id); loadBks(week) } catch (e: any) { setMsg('✖ ' + e.message) }
  }
  async function piggyback(bookingId: number) {
    setMsg('')
    try { await http.post('/instruments/piggyback', { bookingId }); setMsg('✅ 已申请蹭一下，等对方在「我的仪器」通过') } catch (e: any) { setMsg('✖ ' + e.message) }
  }

  const days = week ? Array.from({ length: 7 }, (_, i) => addDays(week, i)) : []
  const curCat = cats.find((c) => c.key === cat)
  const recordMode = curCat ? !curCat.lottery : false
  const catInsts = insts.filter((i) => i.category === cat)
  const tabs = [...cats].sort((a, b) => (a.lottery === b.lottery ? a.sortOrder - b.sortOrder : (a.lottery ? -1 : 1)))

  function Cell({ inst, date }: { inst: Inst; date: string }) {
    const dayBks = bks.filter((b) => b.instrumentId === inst.id && b.date === date)
    const lay = LAYOUT[inst.blockType] || LAYOUT.FOUR_HOUR
    const canLottery = isStudent && (inst.category !== 'POLY_HEAD' || myHeads.includes(inst.id))
    let any = false
    const rows = lay.map(([s, e]) => {
      const b = dayBks.find((x) => x.startHour === s)
      if (b) {
        any = true
        const isRec = b.source === 'RECORD'
        const color = isRec ? 'bg-slate-200 text-slate-700' : b.source === 'CLAIM' ? 'bg-green-100 text-green-800' : 'bg-primary/10 text-primary'
        return (
          <div key={s} className={'mb-0.5 rounded px-1 py-0.5 text-xs ' + color}>
            {b.startHour}–{b.endHour} {isRec ? '使用' : (TASK[b.taskType] || '')}·{b.userName}{b.tempCeiling ? `(${b.tempCeiling}℃)` : ''}{b.fromName ? ` ←${b.fromName}` : ''}
            {(b.source === 'CLAIM' || b.source === 'RECORD') && (b.userId === myId || isAdmin) && <button onClick={() => release(b)} className="ml-1 text-red-500">✖</button>}
            {inst.piggyback && isStudent && b.userId !== myId && !isRec && (piggConfirm === b.id
              ? <span className="ml-1 whitespace-nowrap"><button onClick={() => { piggyback(b.id); setPiggConfirm(null) }} className="rounded bg-green-600 px-1 text-white">确定</button><button onClick={() => setPiggConfirm(null)} className="ml-0.5 rounded bg-slate-300 px-1">取消</button></span>
              : <button onClick={() => setPiggConfirm(b.id)} className="ml-1 rounded bg-amber-200 px-1 text-amber-900">蹭</button>)}
          </div>
        )
      }
      const covered = dayBks.some((x) => x.startHour < e && s < x.endHour)
      if (covered) return null
      if (recordMode) {
        if (!user) return null
        any = true
        return <button key={s} onClick={() => claim(inst, date, s, e)} className="mb-0.5 block w-full rounded border border-dashed border-slate-400 px-1 text-left text-xs text-slate-600 hover:bg-slate-100">登记 {s}–{e}</button>
      }
      if (canLottery) {
        any = true
        const past = new Date(date + 'T' + String(s).padStart(2, '0') + ':00:00').getTime() < Date.now()
        return past
          ? <div key={s} className="mb-0.5 px-1 text-xs text-muted-foreground/70">已过期 {s}–{e}</div>
          : <button key={s} onClick={() => claim(inst, date, s, e)} className="mb-0.5 block w-full rounded border border-dashed border-accent/60 px-1 text-left text-xs text-accent hover:bg-accent/10">可约 {s}–{e}</button>
      }
      return null
    })
    return <td className="border border-border p-1 align-top">{rows}{!any && <span className="text-xs text-muted-foreground">—</span>}</td>
  }

  return (
    <div className="space-y-4">
      <Card>
        <CardContent className="flex flex-wrap items-center gap-3 py-3 text-sm">
          <Button variant="outline" className="h-8 px-2" disabled={prevDisabled} onClick={() => go(-7)}>← 上一周</Button>
          <span className="font-medium text-foreground">{week} ~ {week && addDays(week, 6)}{week === today0 ? '（本周）' : week === maxStu ? '（下周）' : ''}</span>
          <Button variant="outline" className="h-8 px-2" disabled={nextDisabled} onClick={() => go(7)}>下一周 →</Button>
          <Button variant="outline" className="h-8 px-2" onClick={() => { setWeek(today0); setMsg('') }}>回到本周</Button>
          {isAdmin && <Button variant="accent" className="h-8" disabled={busy} onClick={runLottery}>对本周运行抽签</Button>}
          <span className="text-muted-foreground">蓝=抽签 · 绿=点击即得 · 灰=使用记录</span>
          {isStudent && <span className="text-muted-foreground">（学生仅可查看本周与下周）</span>}
          {msg && <span className="text-accent">{msg}</span>}
        </CardContent>
      </Card>

      <div className="flex flex-wrap gap-2">
        {tabs.map((c) => (
          <button key={c.key} onClick={() => setCat(c.key)} className={'rounded-md px-3 py-1 text-sm ' + (cat === c.key ? 'bg-primary text-primary-foreground' : 'bg-card border border-border text-foreground') + (c.lottery ? '' : ' italic')}>{c.label}{c.lottery ? '' : '·记录'}</button>
        ))}
      </div>

      <Card>
        <CardHeader><CardTitle>仪器排班表 · {curCat?.label || ''}（{catInsts.length} 台）{recordMode && <span className="ml-2 text-sm font-normal text-muted-foreground">使用记录表：点击「登记」记录使用情况</span>}</CardTitle></CardHeader>
        <CardContent className="overflow-x-auto">
          {isStudent && cat === 'POLY_HEAD' && <p className="mb-2 text-xs text-muted-foreground">只能预约你被授权的机头（其余仅可查看）。</p>}
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr>
                <th className="border border-border bg-primary px-2 py-1 text-left text-primary-foreground">仪器</th>
                {days.map((d, i) => <th key={d} className="border border-border bg-primary px-2 py-1 text-left text-primary-foreground">{WD[i]}<br />{d.slice(5)}</th>)}
              </tr>
            </thead>
            <tbody>
              {catInsts.map((inst) => (
                <tr key={inst.id}>
                  <td className="border border-border bg-primary/5 px-2 py-1 align-top text-xs font-medium text-primary">{inst.name}</td>
                  {days.map((d) => <Cell key={d} inst={inst} date={d} />)}
                </tr>
              ))}
              {catInsts.length === 0 && <tr><td colSpan={8} className="border border-border py-4 text-center text-muted-foreground">该大类暂无仪器</td></tr>}
            </tbody>
          </table>
        </CardContent>
      </Card>
    </div>
  )
}
""")

# ===== InstrumentAdmin.tsx =====
wfile(W + "/src/pages/InstrumentAdmin.tsx", r"""import { useEffect, useState } from 'react'
import http from '../lib/api'
import { useAuth } from '../store/auth'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'
import { Button } from '@/components/ui/button'

const BT_MAX: Record<string, number> = { '全天': 7, '半天': 14, '四块': 28, '3+1': 8 }
export default function InstrumentAdmin() {
  const { user } = useAuth()
  const [cats, setCats] = useState<any[]>([])
  const [insts, setInsts] = useState<any[]>([])
  const [f, setF] = useState({ name: '', mode: 'EXISTING', categoryKey: 'VACUUM_OVEN', newLabel: '', blockType: '四块', lottery: true, unlimited: false, weeklyCap: '', location: '' })
  const [msg, setMsg] = useState('')
  async function load() { const c: any = await http.get('/instruments/categories'); setCats(c); const i: any = await http.get('/instruments'); setInsts(i) }
  useEffect(() => { load() }, [])
  if (user?.role !== 'ADMIN') return <Card><CardContent className="py-10 text-center text-muted-foreground">仅管理员可添加仪器。</CardContent></Card>
  const exist = cats.find((c) => c.key === f.categoryKey)
  const lotteryEff = f.mode === 'OTHER' ? false : f.mode === 'EXISTING' ? (exist?.lottery ?? false) : f.lottery
  const showCap = lotteryEff && (f.mode === 'NEW' || (f.mode === 'EXISTING' && exist && !exist.builtin))
  async function submit() {
    setMsg('')
    if (!f.name.trim()) { setMsg('✖ 请输入设备名称'); return }
    const body: any = { name: f.name, categoryMode: f.mode, blockType: f.blockType, location: f.location }
    if (f.mode === 'EXISTING') body.categoryKey = f.categoryKey
    if (f.mode === 'NEW') { body.newCategoryLabel = f.newLabel; body.lottery = f.lottery }
    if (showCap) body.weeklyCap = f.unlimited ? null : (f.weeklyCap || null)
    try { await http.post('/instruments/admin/instrument', body); setMsg('✅ 已添加'); setF({ ...f, name: '', newLabel: '' }); load() }
    catch (e: any) { setMsg('✖ ' + e.message) }
  }
  const grouped = cats.map((c) => ({ c, items: insts.filter((i) => i.category === c.key) })).filter((g) => g.items.length)
  return (
    <div className="space-y-4">
      <Card className="max-w-2xl">
        <CardHeader><CardTitle>添加仪器（仅管理员）</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <div><Label className="text-xs">设备名称 *</Label><Input className="h-9" value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} /></div>
          <div>
            <Label className="text-xs">大类</Label>
            <div className="mt-1 flex flex-wrap gap-3 text-sm">
              {[['EXISTING', '已有大类'], ['OTHER', '其他（仅记录，不抽签）'], ['NEW', '创建新大类']].map(([k, t]) => (
                <label key={k} className="flex items-center gap-1"><input type="radio" checked={f.mode === k} onChange={() => setF({ ...f, mode: k })} />{t}</label>
              ))}
            </div>
            {f.mode === 'EXISTING' && <select className="mt-2 h-9 rounded-md border border-input bg-background px-2 text-sm" value={f.categoryKey} onChange={(e) => setF({ ...f, categoryKey: e.target.value })}>{cats.map((c) => <option key={c.key} value={c.key}>{c.label}{c.lottery ? '（抽签）' : '（记录）'}</option>)}</select>}
            {f.mode === 'NEW' && <Input className="mt-2 h-9 w-60" placeholder="新大类名称，如 纺丝机" value={f.newLabel} onChange={(e) => setF({ ...f, newLabel: e.target.value })} />}
          </div>
          <div className="flex flex-wrap items-end gap-4">
            <div><Label className="text-xs">分块类型</Label><select className="h-9 rounded-md border border-input bg-background px-2 text-sm" value={f.blockType} onChange={(e) => setF({ ...f, blockType: e.target.value })}>{['四块', '半天', '全天', '3+1'].map((b) => <option key={b} value={b}>{b}</option>)}</select></div>
            {f.mode === 'NEW' && <label className="flex items-center gap-1 text-sm"><input type="checkbox" checked={f.lottery} onChange={(e) => setF({ ...f, lottery: e.target.checked })} />参与抽签</label>}
            <span className="text-xs text-muted-foreground">四块=每天4个4h格 · 半天=上/下午 · 全天=整天 · 3+1=真空式(铺膜整天+干燥4h)</span>
          </div>
          {showCap && (
            <div className="flex flex-wrap items-end gap-3">
              <div><Label className="text-xs">每人每周报名上限（块）</Label><Input className="h-9 w-32" type="number" disabled={f.unlimited} value={f.weeklyCap} onChange={(e) => setF({ ...f, weeklyCap: e.target.value })} placeholder={'最大 ' + (BT_MAX[f.blockType] || 28)} /></div>
              <label className="flex items-center gap-1 text-sm"><input type="checkbox" checked={f.unlimited} onChange={(e) => setF({ ...f, unlimited: e.target.checked })} />不限制</label>
              <span className="text-xs text-muted-foreground">单台每周共 {BT_MAX[f.blockType] || 28} 块（{f.blockType}）</span>
            </div>
          )}
          <div><Label className="text-xs">存放位置（选填）</Label><Input className="h-9 w-60" value={f.location} onChange={(e) => setF({ ...f, location: e.target.value })} /></div>
          <div className="flex items-center gap-3"><Button onClick={submit}>添加仪器</Button>{msg && <span className="text-sm text-accent">{msg}</span>}</div>
        </CardContent>
      </Card>

      <Card>
        <CardHeader className="pb-2"><CardTitle>现有仪器（{insts.length} 台）</CardTitle></CardHeader>
        <CardContent className="space-y-2">
          {grouped.map((g) => (
            <div key={g.c.key} className="text-sm">
              <span className="font-medium text-foreground">{g.c.label}</span> <span className="text-xs text-muted-foreground">{g.c.lottery ? '抽签' : '记录'} · {g.items.length} 台</span>
              <div className="mt-0.5 text-xs text-muted-foreground">{g.items.map((i: any) => i.name).join('、')}</div>
            </div>
          ))}
        </CardContent>
      </Card>
    </div>
  )
}
""")

# ===== Signup.tsx 自定义大类支持 =====
pyedit(W + "/src/pages/Signup.tsx", [
  ["  const [showSum, setShowSum] = useState(false)",
   "  const [showSum, setShowSum] = useState(false)\n  const [cats, setCats] = useState<any[]>([])\n  const [custom, setCustom] = useState<Record<string, number>>({})"],
  ["    http.get('/instruments/forecast').then((d: any) => setFc(d)).catch(() => {})",
   "    http.get('/instruments/forecast').then((d: any) => setFc(d)).catch(() => {})\n    http.get('/instruments/categories').then((d: any) => setCats(d)).catch(() => {})"],
  ["""    setSm({ MUFFLE: g('MUFFLE')?.blockCount || 0, TUBE: g('TUBE')?.blockCount || 0, BET: g('BET')?.blockCount || 0, DMA: g('DMA')?.blockCount || 0, TGA: g('TGA')?.blockCount || 0 })
  }, [mine])""",
   """    setSm({ MUFFLE: g('MUFFLE')?.blockCount || 0, TUBE: g('TUBE')?.blockCount || 0, BET: g('BET')?.blockCount || 0, DMA: g('DMA')?.blockCount || 0, TGA: g('TGA')?.blockCount || 0 })
  }, [mine])
  useEffect(() => {
    const cc: Record<string, number> = {}
    cats.filter((c) => c.lottery && !c.builtin).forEach((c) => { cc[c.key] = mine.find((d) => d.category === c.key)?.blockCount || 0 })
    setCustom(cc)
  }, [mine, cats])"""],
  ["""      { category: 'DMA', blockCount: sm.DMA }, { category: 'TGA', blockCount: sm.TGA },
    ]""",
   """      { category: 'DMA', blockCount: sm.DMA }, { category: 'TGA', blockCount: sm.TGA },
      ...cats.filter((c) => c.lottery && !c.builtin).map((c) => ({ category: c.key, blockCount: custom[c.key] || 0 })),
    ]"""],
  ["      {/* 总保存 + 查看 */}",
   """      {cats.filter((c) => c.lottery && !c.builtin).length > 0 && (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {cats.filter((c) => c.lottery && !c.builtin).map((c) => (
            <Card key={c.key}><CardContent className="space-y-1 py-3">
              <div className="text-sm font-medium text-foreground">{c.label}</div>
              <div className="text-xs text-muted-foreground">{c.weeklyCap == null ? '不限' : '上限 ' + c.weeklyCap} · 自定义大类</div>
              <select className={sel0} value={custom[c.key] || 0} onChange={(e) => setCustom({ ...custom, [c.key]: +e.target.value })}>{range(0, c.weeklyCap == null ? 28 : c.weeklyCap).map((n) => <option key={n} value={n}>{n}</option>)}</select>
            </CardContent></Card>
          ))}
        </div>
      )}
      {/* 总保存 + 查看 */}"""],
  ["{CAT_LABEL[d.category] || d.category}",
   "{CAT_LABEL[d.category] || cats.find((c) => c.key === d.category)?.label || d.category}"],
])

# ===== App.tsx 路由 + 导航 =====
pyedit(W + "/src/App.tsx", [
  ["import Overview from './pages/Overview'",
   "import Overview from './pages/Overview'\nimport InstrumentAdmin from './pages/InstrumentAdmin'"],
  ["        <Route path=\"overview\" element={<Overview />} />",
   "        <Route path=\"overview\" element={<Overview />} />\n        <Route path=\"instrument-admin\" element={<InstrumentAdmin />} />"],
])
pyedit(W + "/src/layouts/MainLayout.tsx", [
  ["            {user?.role === 'ADMIN' && <NavLink to=\"/priorities\" className={linkCls}>优先级</NavLink>}",
   "            {user?.role === 'ADMIN' && <NavLink to=\"/instrument-admin\" className={linkCls}>添加仪器</NavLink>}\n            {user?.role === 'ADMIN' && <NavLink to=\"/priorities\" className={linkCls}>优先级</NavLink>}"],
])

step("构建前端", PATHX + "cd %s && NODE_OPTIONS=--max-old-space-size=1536 npm run build 2>&1 | tail -10"%W, 520)
step("部署", "rm -rf %s/index.html %s/assets && cp -rf %s/dist/* %s/ && chown -R www:www %s 2>/dev/null; curl -s http://127.0.0.1:8080/ | grep -o '/assets/[^\"]*[.]js' | head -1"%(SITE,SITE,W,SITE,SITE))
cli.close(); print("\n=== DONE ===")
